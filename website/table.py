...
SIDES = {"Orit":"Orit's Family", "Edo":"Edo's Family"}
def table(request):
    guest_side = random.choice(list(SIDES.keys()))
    random_guest = models.Guest.objects.filter(table_name=SIDES[guest_side]).order_by('?').first()
    ...
    # else:
    #     form = ReceptionForm()
    return render(request, 'table.pug', { 'guest' : random_guest, 'guest_side': guest_side, 'footer_info': urls.footer_info(request)})
